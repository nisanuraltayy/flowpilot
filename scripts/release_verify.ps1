...
try {
  ...
  Invoke-Gate "Backend: alembic current == 0008" {
    $prev = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    $out = (& $Python -m alembic -c apps/backend/alembic.ini current 2>&1 | Out-String)
    $ErrorActionPreference = $prev
    $revLine = ($out -split "`n" | Where-Object { $_ -match "0008" }) -join " "
    Write-Host ("alembic current: {0}" -f $revLine.Trim())
    if ($out -match "0008") { $global:LASTEXITCODE = 0 } else { $global:LASTEXITCODE = 1 }
  }
